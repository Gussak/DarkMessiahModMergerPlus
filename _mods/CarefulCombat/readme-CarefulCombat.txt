...
SelfNote: Vanilla total skill points to magic: 54 
#54 #(cd ../../../;strVFl="Dark Messiah Might and Magic Single Player.layer005.VanillaExtractedTextFiles.IGNORE_LAYER/mm/scripts/mm_skills_infos.txt";bc <<< "$(cat "${strVFl}" |egrep "magic|cost" |egrep -v "module|require|^//|\"(magic_2|magic_10)\"" |grep magic -A 1|grep cost |awk '{print $2}' |tr -d '"\r' |tr '\n' '+')0") #"
# #(strVFl="content/scripts/mm_skills_infos.txt";bc <<< "$(cat "${strVFl}" |egrep "magic|cost" |egrep -v "module|require|^//|\"(magic_2|magic_10)\"" |grep magic -A 1|grep cost |awk '{print $2}' |tr -d '"\r' |tr '\n' '+')0") #"
